...
(function () {
  ...
  function markProvenance() {
    if (!window.Flotzy || !window.Flotzy.isReal) return;
    document.querySelectorAll('.specimen[id]').forEach(function (spec) {
      ...
      var real = window.Flotzy.isReal(id);
      var host = spec.querySelector('.specimen__id');
      if (!host || host.querySelector('.tag')) return;
      var tag = document.createElement('span');
      tag.className = 'tag ' + (real ? 'tag--meth' : 'tag--steam');
      tag.style.marginLeft = '.5rem';
      tag.textContent = real ? 'Recorded' : 'Synthesised';
      tag.title = real
        ? 'A real recording, sourced from Wikimedia Commons. See provenance below.'
        : 'No suitable recording exists for this class; generated by the reed-valve model.';
